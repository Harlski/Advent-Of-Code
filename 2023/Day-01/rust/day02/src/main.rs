...
fn main() {
    ...
    for line in lines {
        let (game_num, games) = split_lines(line); 
        if !contains_higher(games) {
...
fn contains_higher(games: Vec<Vec<String>>) -> bool {
...
fn split_lines(v: String) ->  (String, Vec<Vec<String>>) { // The idea here is to pass each line of file after regex and separate each drawing into its own array/vec.
